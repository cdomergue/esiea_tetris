...
package main.java.domergue.bastide.jTetris.librairies;
...
public class RawConsoleInput {
	 ...
	 *         available. -1 on EOF. Otherwise an Unicode character code within
	...
	private static int readUnix(boolean wait) throws IOException {
		...
		consoleModeAltered = true;
		setTerminalAttrs(stdinFd, rawTermios); // switch off canonical mode,
												// echo and signals
		try {
	...
	// (This method is synchronized because the charsetDecoder must only be used
	...
	private static synchronized void initUnix() throws IOException {
		...
		if (stdinIsConsole) {
			...
			// Canonical mode can be switched off between the read() calls, but
			// echo must remain disabled.
			registerShutdownHook();
		}
		initDone = true;
	}
	...
	protected static class Termios extends Structure { // termios.h
		...
		public int c_lflag;
		public byte c_line;
		public byte[] filler = new byte[64]; // actual length is platform
												// dependent
